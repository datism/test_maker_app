...
import { ChevronLeft, Edit, Plus, GripVertical, Trash2 } from 'lucide-react';

const TestPreview = ({ test, onBack, onEdit }) => {
  const questions = [];
            ...
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-3xl font-bold text-gray-900">{test?.name || 'Untitled Test'}</h1>
                </div>
                <button onClick={() => onEdit && onEdit(test)} className="flex items-center gap-2 px-4 py-2 text-gray-700 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
                  <Edit size={18} />
              ...
              <button className="w-full py-4 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors font-semibold text-lg shadow-md hover:shadow-lg">Generate Test</button>
            </div>
          </div>
        </div>
      </div>
